...
app.post('/insert', (req, res) => {
    const sql = 'INSERT into songs (sname, composer, lvl1, lvl2, lvl3, lvl4, lvl4name) values (?, ?, ?, ?, ?, ?, ?)'
    const song = [
      req.body.sname,
      req.body.composer,
      req.body.lvl1,
...
app.post('/modify', (req, res) => {
  const sql = 'UPDATE songs SET sname = ?, composer = ?, lvl1 = ?, lvl2 = ?, lvl3 = ?, lvl4 = ?, lvl4name = ? WHERE sname = ?'
  const song = [
    req.body.sname_new,
    req.body.composer,
    req.body.lvl1,
...
app.post("/addscore", (req, res) => {
  const sql = 'INSERT INTO scores (player_id, song_id, lvl1_score, lvl2_score, lvl3_score, lvl4_score) \
  SELECT p.id, s.id, ?, ?, ?, ? \
  FROM players p \
  JOIN songs s ON s.sname = ? \
  WHERE p.username = ?;'
  const information = [
    req.body.lvl1,
    req.body.lvl2,
...
app.get("/getscore", (req, res) => {
  const sql = "select songs.id, sname, composer, lvl4name, \
  lvl1_score, lvl2_score, lvl3_score, lvl4_score, username, volforce from songs \
  left join scores on scores.song_id = songs.id left join players on scores.player_id = players.id where username is not null"
  db.query(sql, (err, rows) => {
    if (err) {
      res.json({result: "error"})
      return console.log(err)
